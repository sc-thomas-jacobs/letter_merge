...
class MailMergeApp:
    ...
    def setup_styles(self) -> None:
        ...
        style.configure("TFrame", background="#0A3D62")
        style.configure("TLabel", background="#0A3D62", foreground="white", font=("Segoe UI", 11))
        style.configure("Header.TLabel", font=("Segoe UI", 14, "bold"), foreground="white", background="#0A3D62")
        style.configure("TButton", background="#1B4F72", foreground="white", font=("Segoe UI", 10, "bold"), padding=6)
        style.map("TButton", background=[("active", "#154360")])
        style.configure("TCombobox", fieldbackground="#F2F4F4", background="#F2F4F4", padding=4)
    ...
    def create_widgets(self) -> None:
        ...
        ttk.Label(frame, text="Schools Choice Mail Merge", style="Header.TLabel").grid(row=0, column=0, columnspan=2, pady=(0, 20))

        ttk.Label(frame, text="Select Data File:").grid(row=1, column=0, sticky="w", pady=5)
        ttk.Button(frame, text="Browse", command=self.select_data_file).grid(row=1, column=1, pady=5)

        ttk.Label(frame, text="Select Template File:").grid(row=2, column=0, sticky="w", pady=5)
        ttk.Button(frame, text="Browse", command=self.select_template_file).grid(row=2, column=1, pady=5)

        ttk.Label(frame, text="Select Output Folder:").grid(row=3, column=0, sticky="w", pady=5)
        ttk.Button(frame, text="Browse", command=self.select_output_folder).grid(row=3, column=1, pady=5)

        ttk.Label(frame, text="Grouping Column:").grid(row=4, column=0, sticky="w", pady=5)
        self.grouping_dropdown = ttk.Combobox(frame, textvariable=self.grouping_column, state="readonly", width=30)
        self.grouping_dropdown.grid(row=4, column=1, pady=5)

        ttk.Button(frame, text="Run Mail Merge", command=self.run_mail_merge).grid(row=5, column=0, columnspan=2, pady=20)
