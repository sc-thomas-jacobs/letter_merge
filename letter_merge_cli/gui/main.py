...
class LetterMergeApp(ctk.CTk):
    ...
    def update_grouping_options(self, excel_path):
        try:
    ...
    def run_logic(self):
        data_path = self.data_picker.get()
        template_path = self.template_picker.get()
        output_path = self.output_picker.get()
        standard_name = self.name_entry.get()
        grouping_type = self.group_dropdown.get()

        filename_pattern = f"{standard_name}_{{{grouping_type}}}.docx"
        ...
        print(f"Grouping Column: {grouping_type}")
        ...
        try:
            ...
            core.generate_documents(self.data, Path(template_path), Path(output_path), grouping_type, self.name_entry.get())
